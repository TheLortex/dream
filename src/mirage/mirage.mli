...
val initialize_log :
  ?backtraces:bool -> ?level:[< log_level] -> ?enable:bool -> unit -> unit
...
val error_template : (error -> string -> response -> response) -> error_handler
...
val http :
  ...
  < net : Eio.Net.t ; clock : Eio.Time.clock ; .. > ->
  ?error_handler:error_handler ->
  handler ->
  unit
